...
# where the octave fits into 9 bits
START_NOTE = 72
...
for n in range(0, 12):
    fnums.append([])
    for s in range(0, STEP_COUNT):
        fnum = round(((midi_note_frequencies[n][s] * math.pow(2.0, 18)) / MUSIC_FM_FSAM) / math.pow(2, 3))
        fnums[n].append(fnum)

outfile = open("music_driver/opl_2op/fnums_fm.inc", "w")

outfile.write("opl_2op_tone_lookup:\n")
