...
for n in range(0, 12):
    fnums.append([])
    for s in range(0, STEP_COUNT):
        freq = round(MUSIC_CLOCK / ( 2 * 16 * midi_note_frequencies[n][s]))
        freq = 1023 if freq > 1023 else freq
        fnums[n].append(freq)

outfile = open("music_driver/sn/fnums_sn.inc", "w")

outfile.write("sn_tone_lookup:\n")
